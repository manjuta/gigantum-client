...
class TestJobs(object):
    def test_success_import_export_zip(self, mock_config_with_repo):
        config_instance, working_dir = mock_config_with_repo
        ...
        with tempfile.TemporaryDirectory() as temp_dir_path:
            # Export the labbook
            export_dir = os.path.join(config_instance.app_workdir, "export")
            ...
            import_lb = InventoryManager().load_labbook_from_directory(imported_lb_path)
            ...
            import_lb2 = InventoryManager().load_labbook_from_directory(user_import_lb)
    ...
    def test_import_labbook_from_remote(self, mock_config_with_repo, monkeypatch):
        def _mock_import_labbook_from_remote(remote, username):
            print('X' * 200)
            lb = InventoryManager().create_labbook(username, username, remote.repo_name)
    ...
    def test_fail_import_export_zip(self, mock_config_with_repo):
        ...
        # Create new LabBook to be exported
        lb = InventoryManager().create_labbook('test', 'test', "lb-fail-export-import-test",
    ...
    @mock.patch('gtmcore.workflows.gitworkflows_utils.create_remote_gitlab_repo', new=helper_create_remote_repo)
    def test_publish_respository(self, mock_labbook_lfs_disabled):
        ...
        # In practice we need to serialize the LB instance when actually scheduling, so we should test that way.
