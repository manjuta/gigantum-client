...
import os
import requests
from gtmcore.activity.tests.fixtures import mock_redis_client, MockSessionsResponse
...
class TestJupyterLabMonitor(object):
    ...
    def test_supported_names(self, mock_redis_client):
    ...
    def test_get_sessions(self, mock_redis_client, monkeypatch):
        ...
        mock_redis_client.set(f"{lb_key}-jupyter-token", "afaketoken")
        mock_redis_client.hset(dev_env_key, "url", "http://localhost:10000/jupyter/asdf/")

        sessions = monitor.get_sessions(dev_env_key, redis_conn=mock_redis_client)
    ...
    def test_run(self, mock_redis_client, monkeypatch):
        ...
        lb_key = project_info.default_image_tag('default', 'test-labbook')
        mock_redis_client.set(f"{lb_key}-jupyter-token", "afaketoken")
        mock_redis_client.hset(dev_env_key, "author_name", "default")
        mock_redis_client.hset(dev_env_key, "author_email", "default@default.io")
        mock_redis_client.hset(dev_env_key, "url", "http://localhost:10000/jupyter/asdf/")

        monitor.run(dev_env_key)

        data = mock_redis_client.hgetall('{}:activity_monitor:6e529520-2a6d-4adb-a2a1-de10b85b86a6'.format(dev_env_key))
        ...
        data = mock_redis_client.hgetall('{}:activity_monitor:146444cd-b4ec-4c00-b29c-d8cef4a503b0'.format(dev_env_key))
