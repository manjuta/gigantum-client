...
class IOManager(object):
    ...
    def push_objects(self, status_update_fn: Callable = None) -> PushResult:
        ...
        requests.post(
            f"https://{tracking_service}/repo/{self.dataset.namespace}/{self.dataset.name}/data_size/{total_size}",
    ...
    def pull_objects(self, keys: List[str], status_update_fn: Callable = None) -> PullResult:
        ...
        for successful_pull in result.success:
            ...
            tracking_service = 'yvnb2ma8id.execute-api.us-east-1.amazonaws.com/api'
            requests.post(
                f"https://{tracking_service}/object_download/{self.dataset.namespace}/{self.dataset.name}/{object_id}",
                headers={"token": "d43f80mbvdsrju567ubg"})
