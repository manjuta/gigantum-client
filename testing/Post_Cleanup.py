...
from misc import (gqlquery as run_query, endpt_get, endpt_post, USERNAME,
...
if __name__ == '__main__':
    ...
    d = run_query(endpoint, 'Get Local Labbooks', localLabbookQuery, {})
    local_lbs = d['data']['labbookList']['localLabbooks']['edges']
    for llb in local_lbs:
        if 'cli-' in llb['node']['name']:
            d = run_query(endpoint, f'Delete Local {llb["node"]["owner"]}/{llb["node"]["name"]}',
                    deleteLocalLabbook,
                    {'owner': llb["node"]["owner"],
                     'lbname': llb["node"]["name"]})
    
    d = run_query(endpoint, 'Get Remote Labbooks', remoteLabbookQuery, {})
    remote_lbs = d['data']['labbookList']['remoteLabbooks']['edges']
    for rlb in remote_lbs:
        if 'cli-' in rlb['node']['name']:
            print(rlb)
            d = run_query(endpt_get, f'Delete Remote {rlb["node"]["owner"]}/{rlb["node"]["name"]}',
                    deleteRemoteLabbook,
